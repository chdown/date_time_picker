...
class BrnMultiRangePickerCommonItem extends StatelessWidget {
    ...
    if (!item.isUnLimit() && (item.children.isEmpty)) {
      if (item.isInLastLevel() && _hasCheckBoxBrother(item)) {
        checkbox = Container(
          ...
          width: 21,
          child: (item.isSelected) ? Icon(Icons.check_circle_outline, color: Colors.green) : Icon(Icons.radio_button_unchecked),
        );
      } else {
        checkbox = Container();
      }
            ...
            children: <Widget>[
              Container(
                ...
                child: Text(
                  ...
                  style: TextStyle(fontSize: 15, fontWeight: _getItemFontWeight(), decoration: TextDecoration.none, color: _getItemTextColor()),
                ),
              )),
              checkbox
            ],
  ...
  Color _getItemTextColor() {
    Color itemColor = (item.isUnLimit() ? isCurrentFocused : item.isSelected)! ? selectColor : normalColor;
  ...
  FontWeight _getItemFontWeight() {
    FontWeight fontWeight = (item.isUnLimit() ? isCurrentFocused : item.isSelected)! ? FontWeight.w600 : FontWeight.normal;
  ...
  bool _hasCheckBoxBrother(BrnPickerEntity item) {
    int? count;
    if (item.parent != null) {
      count = item.parent!.children.where((f) => f.filterType == PickerFilterType.checkbox).length;
    }
