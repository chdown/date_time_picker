...
import 'package:date_time_picker/picker/multi_range_picker/brn_multi_column_picker.dart';
import 'package:date_time_picker/picker/multi_range_picker/brn_multi_column_picker_util.dart';
import 'package:date_time_picker/picker/multi_range_picker/btn_multi_column_picker_item.dart';
...
/// [index] 点击位置处于当前列的位置
/// [entity] 被点击位置的数据
typedef BrnOnSelectEntityInterceptor = bool Function(int? listIndex, int index, BrnPickerEntity entity);
...
class BrnMultiColumnListWidget extends StatefulWidget {
  ...
  BrnMultiColumnListWidget({
    ...
    currentListIndex = BrnMultiColumnPickerUtil.getCurrentColumnIndex(items!.isNotEmpty ? items![0] : null);
...
class _BrnMultiColumnListWidgetState extends State<BrnMultiColumnListWidget> {
  ...
  Widget build(BuildContext context) {
    return Expanded(
      flex: widget.flex,
      child: Container(
        constraints: (widget.maxHeight == 0) ? BoxConstraints.expand() : BoxConstraints(maxHeight: widget.maxHeight),
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            return BrnMultiRangePickerCommonItem(
              ...
              itemSelectFunction: (BrnPickerEntity entity) {
                if (widget.onSelectEntityInterceptor != null && widget.onSelectEntityInterceptor!(widget.currentListIndex, index, entity) == false) {
                  return;
                }
  ...
  void _processFilterData(BrnPickerEntity? selectedEntity) {
    ...
    int totalLevel = BrnMultiColumnPickerUtil.getTotalColumnCount(selectedEntity);
    ...
    if (widget.items != null && widget.items!.isNotEmpty && widget.items![0].parent != null) {
      widget.items![0].parent?.isSelected = widget.items![0].parent!.children.where((BrnPickerEntity f) => f.isSelected).isNotEmpty;
    }
