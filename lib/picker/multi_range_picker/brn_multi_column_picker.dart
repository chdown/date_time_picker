...
import 'package:date_time_picker/picker/multi_range_picker/bean/brn_multi_column_picker_entity.dart';
import 'package:date_time_picker/picker/multi_range_picker/brn_multi_column_converter.dart';
import 'package:date_time_picker/picker/multi_range_picker/brn_multi_column_list.dart';
import 'package:date_time_picker/picker/multi_range_picker/brn_multi_column_picker_util.dart';
...
/// [rowIndex] 数据项所在列中的位置
/// [entity] 被点击的数据项
typedef BrnOnEntityTap = void Function(int columnIndex, int rowIndex, BrnPickerEntity entity);
...
typedef BrnOnPickerConfirm = void Function(Map<String, List<BrnPickerEntity>> results, int? firstIndex, int? secondIndex, int? thirdIndex);
...
class _BrnSelectionGroupViewState extends State<BrnMultiColumnPicker> {
  static const BrnMultiRangeSelConverter defaultConverter = const BrnMultiRangeSelConverter();
  ...
  @override
  void initState() {
    _normalColor = BrnPickerConfig.colorTextBase;
    _selectedColor = BrnPickerConfig.colorPrimary;
    _initData();
    super.initState();
  }
  ...
  Widget _listWidget() {
    ...
    if (!BrunoTools.isEmpty(_firstList) && BrunoTools.isEmpty(_secondList) && BrunoTools.isEmpty(_thirdList)) {
      /// 1、仅有一级的情况
      /// 1.1 一级单选 && 没有自定义范围的情况
      widgets.add(BrnMultiColumnListWidget(
          ...
          singleListItemPick: (int listIndex, int index, BrnPickerEntity entity) {
            _setFirstIndex(index);
            if (_columnCount == 1 && widget.entity.filterType == PickerFilterType.radio) {
              _confirmButtonClickEvent();
            }
    ...
    } else if (!BrunoTools.isEmpty(_firstList) && !BrunoTools.isEmpty(_secondList) && BrunoTools.isEmpty(_thirdList)) {
      /// 2、有二级的情况
      widgets.add(BrnMultiColumnListWidget(
          items: _firstList,
          onSelectEntityInterceptor: widget.canSelectEntryInterceptor,
  ...
  void _confirmButtonClickEvent() {
    if (widget.onConfirm != null) {
      ...
      Map<String, List<BrnPickerEntity>> result = defaultConverter.convertPickedData([widget.entity], includeUnlimitSelection: widget.isIncludeUnLimit);
  ...
  void _configDefaultSelectedData() {
    ...
    if (_firstIndex == -1) {
      if (widget.defaultFocusedIndexes != null) {
        for (int index = 0; index < widget.defaultFocusedIndexes!.length; index++) {
  ...
  void _setFirstIndex(int firstIndex) {
    ...
    if (widget.entity.children.length > _firstIndex!) {
      List<BrnPickerEntity> seconds = widget.entity.children[_firstIndex!].children;
  ...
  int _getInitialSelectIndex(List<BrnPickerEntity>? levelList) {
    ...
    if (index < 0) {
      for (BrnPickerEntity entity in levelList) {
        ...
        if (entity.isUnLimit() && entity.parent!.filterType != PickerFilterType.checkbox) {
          index = levelList.indexOf(entity);
          break;
        }
      }
