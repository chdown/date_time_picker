...
import 'base/brn_picker_config.dart';
import 'base/brn_picker_title.dart';
import 'base/brn_picker_title_config.dart';
import 'brn_picker_cliprrect.dart';
...
typedef BrnMultiDataPickerCreateWidgetCallback = Widget Function(bool isSelect, int column, int row, List selectedItems);
...
class _BrnMultiDataPickerState extends State<BrnMultiDataPicker> {
  ...
  Widget _configSinglePicker(int component) {
    return MyPicker(
      ...
      createWidgetList: () {
        if (widget.createItemWidget != null) {
          ...
          for (int i = 0; i < widget.delegate.numberOfRowsInComponent(component); i++) {
            ...
            widgetList.add(widget.createItemWidget != null ? widget.createItemWidget!(isSelect, component, i, _selectedIndexList) : Container());
          }
          return widgetList;
        } else {
          List<Widget> list = [];
      ...
      itemExtent: widget.delegate.rowHeightForComponent(component) ?? BrnPickerConfig.itemHeight,
      changed: (int index) {
...
class BrnDefaultMultiDataPickerDelegate implements BrnMultiDataPickerDelegate {
  ...
  BrnDefaultMultiDataPickerDelegate({required this.data, this.firstSelectedIndex = 0, this.secondSelectedIndex = 0, this.thirdSelectedIndex = 0}) {
    if (data.isNotEmpty) {
  ...
  @override
  double rowHeightForComponent(int component) {
    return BrnPickerConfig.itemHeight;
  }
